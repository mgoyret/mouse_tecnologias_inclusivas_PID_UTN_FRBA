...
def sk_send(msg: list):
    try:
        emulador_socket.send(js.dumps(msg).encode('UTF-8'))
...
if __name__ == '__main__':
    #messagebox.showinfo("Emulador", "El emulador a sido iniciado, corre en segundo plano")
    print(f'{__name__}: iniciado')
    try:
        # crear socket INET (IPV4) STREAM (TCP)
        emulador_socket = sk.socket(sk.AF_INET, sk.SOCK_STREAM)
        print(f'{__name__}: socket creado')
        # vincular socket a host publico y puerto conocido
        emulador_socket.connect((sk.gethostname(), mc.port))
        print(f'{__name__}: socket conectado a {sk.gethostname()}')
        #messagebox.showinfo("Emulador", "Emulador conectado")
        while True:
            ...
            if mc.myTecla != mc.no_key_cod and not kb.is_pressed(mc.exit_key) and not kb.is_pressed(mc.action_key):
                ...
                send_exit()
                break
        emulador_socket.shutdown(0)
        emulador_socket.close()
    except:
    ...
    #messagebox.showinfo("Emulador", "Finaliza la ejecucion del emulador")

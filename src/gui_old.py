...
class Window(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self.lb_rueda_S = tk.Label(
            ruedaLabels, text='rueda_S', bg='pink', font=('bold', 12))
...
#        self.lb_rueda_C.grid(row=1, column=3, padx=2, pady=2, sticky='nesw')
        ...
        # ejecuto el iterador de color en el constructor, para que arranque de una
        self.color()
        ...
        if self.color_flag:
            for i in range(len(self.idx)):
                if self.idx[i].cget('background') == 'pink':
                    ...
                    self.idx[(i+1) % int(len(self.idx))].configure(bg='pink')
                    break
